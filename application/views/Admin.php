			...
			$.ajax({
				...
				success: function(data) {
					if(!data.isError) {
						...
						$("#PaymentEdit_DoneButton").attr('onclick', 'new Payment().View_DoneButton('+ id +')')
					...
					if(PaymentCreate_TypeButton.text().toLowerCase() == "others" || PaymentCreate_TypeButton.text().toLowerCase() == "other") {
						data = {
							Titlebox: PaymentCreate_Titlebox.val(),
						 	SlipType: {
						 		"Number": PPaymentCreate_TypeButton.val(),
							...
							if(!data.isError) {
								var HTML = `
									...
									<div id="PaymentView_ItemID` +data.StoreID+ `" class="d-flex flex-row mr-3 mb-3">
										...
										<div class="d-flex flex-column">
											<button class="form-control material-icons p-1 border mb-1" style="width: 35px; height: 35px; font-size: 15px;" onclick="">edit</button>
		...
		this.Edit_DoneButton = function(id) {
			...
			if(PaymentEdit_Titlebox.val() != "") {
				...
				$.ajax({
					...
					beforeSend: function() {
						PaymentEdit_DoneButton.addClass('hide')
					}
				})
			}
			else alert("Error: Title is Empty!")
